...
PlotterToSvg.prototype._transform = function(chunk, encoding, done) {
  switch (chunk.type) {
    ...
    case "pad":
      this._draw(
        flashPad(this.id, chunk.tool, chunk.x, chunk.y, this._element)
      );
      break;
...
PlotterToSvg.prototype._handleNewPolarity = function(polarity, box) {
  if (this._blockMode) {
    if (this._blockLayerCount === 0 && !this._block.length) {
      this._blockMode =
        polarity === "dark" ? BLOCK_MODE_DARK : BLOCK_MODE_CLEAR;
    }
  ...
  if (this._old_polarity === "dark" && polarity === "clear") {
    this._clearCount = this._clearCount + 1;
    var maskId = this.id + "_clear-" + this._clearCount;

    this._old_polarity = "clear";
    this.layer = [maskLayer(maskId, this.layer, this._element)];
    this._maskId = maskId;
    this._maskBox = box.slice(0);
  } else if (this._old_polarity === "clear" && polarity === "dark") {
    // finish the mask and add it to the defs
    this._old_polarity = "dark";
